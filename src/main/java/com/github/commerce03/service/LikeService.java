...
public class LikeService {
    ...
    @Transactional
    public String createLike(LikeRequestDto likeRequestDto) {
        Integer poId = likeRequestDto.getPoId();
        Optional<Like> like = likeJpaRepository.findByPost_PoId(poId);
        if(like.isEmpty()){
            Post post = postJpaRepository.findById(likeRequestDto.getPoId()).orElseThrow(()->new RuntimeException("게시글이 존재하지 않습니다."));
